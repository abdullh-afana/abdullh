        ...
        <div class="test-item reveal">
          <div class="test-content w-100">
            <div class="test-head d-flex justify-content-between align-items-start">
              ...
              {{-- تعديل الحالة بناءً على وجود أسئلة أم لا --}}
              <span class="status-pill {{ $exam->questions_count > 0 ? 'bg-success' : 'bg-warning' }} text-white">
                {{ $exam->questions_count > 0 ? 'نشط' : 'فارغ' }}
              </span>
            </div>
            
            <div class="d-flex justify-content-between align-items-center mt-3">
              <div class="test-info-icons small text-muted">
                <span class="me-3"><i class="bi bi-question-circle"></i> {{ $exam->questions_count ?? 0 }} سؤال</span>
  ...
  <div class="col-lg-4 order-lg-2">
    <div class="card-box reveal">
      ...
      <div class="subject-list">
        ...
        <div class="subject-item reveal d-flex align-items-center p-2 mb-2 border-bottom">
          <div class="subject-icon blue me-3" style="font-size: 1.5rem;"><i class="bi bi-book"></i></div>
          <div class="subject-content">
            <h6 class="mb-0">{{ $subject->name }}</h6>
            <small class="text-muted">{{ $subject->grade->name ?? 'غير محدد' }}</small>
            <div class="subject-meta small mt-1">
              <span class="badge bg-soft-info text-info">{{ $subject->units->count() }} وحدات</span>
            </div>
          </div>
        </div>
        @endforeach
...
<section class="card-box mt-3 reveal">
    ...
    <div class="activity-row reveal">
        ...
        <div>
            <h6>تم إنشاء اختبار: {{ $lastExam->title }}</h6>
    ...
    <div class="activity-row reveal">
        ...
        <div>
            <h6>تم إضافة أسئلة جديدة لـ {{ $lastQuestion->unit->name ?? 'البنك' }}</h6>
